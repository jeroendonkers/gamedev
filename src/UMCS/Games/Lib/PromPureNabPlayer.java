...
* only shallow pruning 
**/


public class PromPureNabPlayer extends Player {

   public PromPureNabPlayer(Game g, Evaluator[] op, double[] pr, int md) {
      super(g); 
      opponent = op;
   ...
   public PromPureNabPlayer(Game g, Evaluator[] op, int md) 
   { super(g); 
     setOpponent(op);
     maxDepth=md;
   }
   ...
   public String toString() { return "PromPureNabPlayer(depth="+maxDepth+",nopp="+nOpponent+")"; }
      ...
      if (isMax) {  
        ...
        // Apply beta-pruning for opponents
         ...
         while (move != NOMOVE) {
            ...
            boolean prune=true;
            for (int i=0; i<nOpponent; i++) 
               if ((b.get(i)) && bestValop[i]<beta[i]) {
                 prune=false; break;
               }

            if (prune) move=NOMOVE; else move = enum.nextMove();
         ...
         while (move != NOMOVE) {
            ...
            Node mm = getGame().doMove(m,move);
            promSearchResult r = promSearch(mm,depth-1,b,bestValop,pre+"  ");
            getGame().undoMove(mm,move);
