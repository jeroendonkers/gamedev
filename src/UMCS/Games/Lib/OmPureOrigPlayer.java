...
public class OmPureOrigPlayer extends AlphaBetaPlayer {

   public OmPureOrigPlayer(Game g, Evaluator op, int md) 
   { super(g,md); 
     eval = g.getDefaultEvaluator();
     evalop = op;
   }

   public OmPureOrigPlayer(Game g, Evaluator e, Evaluator op, int md) 
   { super(g,md); 
     eval = e; evalop = op;
   }
   ...
   private omSearchResult omSearch(Node m, int depth, short beta, String pre) 
    throws OutOfTimeException {
      ...
      if (isMax) {   // we maximize on player's value but also on opponent value
       ...
       if (bestValop <= beta) {  // now evaluate for bestMove!!!
